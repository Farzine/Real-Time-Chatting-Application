...
function CaptureAudio({hide}) {
  ...
  const sendRecording = async() =>{
    try{
      ...
      if(response.status === 201){
        socket.current.emit("send-msg",{message:response.data.message, to: currentChatUser?.id, from: userInfo?.id});
        dispatch({
  ...
  return <div className="flex text-2xl w-full justify-end items-center">
    ...
    {
      isRecording?
      <div className="text-blue-500 animate-pulse text-center w-60">Recording <span> {recordingDuration}s</span></div>
      :
      <div className="">{recordedAudio && <>{!isPlaying?<FaPlay onClick={handlePlayRecording}/>:<FaStop onClick={handlePauseRecording}/>}</>}</div>
    }
    ...
    <div className="mr-4">
      {
        !isRecording ? <FaMicrophone className="text-gray-500" onClick={handleStartRecording}/>  : <FaPauseCircle className="text-gray-500 ml-2" onClick={handleStopRecording}/>
      }
    </div>
      <div className="">
